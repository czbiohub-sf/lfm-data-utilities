...
def process_folder(folder_path: Path, save_loc: Path, focus_graph_loc: Path):
    ...
    while True:
        ...
        ax1.imshow(
            random.choice(grouped_images[predicted_peak - 2]),
            cmap="gray",
            vmin=0,
            vmax=255,
            interpolation=None,
        )
        ax1.set_title("focus - 2 steps")
        ax1.axis("off")

        ax2 = fig.add_subplot(fig.add_subplot(gs[2, 0]))
        ax2.imshow(
            random.choice(grouped_images[predicted_peak - 4]),
            cmap="gray",
            vmin=0,
            vmax=255,
            interpolation=None,
        )
        ax2.set_title("focus - 4 steps")
        ax2.axis("off")

        ax3 = fig.add_subplot(fig.add_subplot(gs[1, 1]))
        ax3.imshow(
            random.choice(grouped_images[predicted_peak]),
            cmap="gray",
            vmin=0,
            vmax=255,
            interpolation=None,
...
if __name__ == "__main__":
    ...
    for i, folder in enumerate(folders, start=1):
        try:
            ...
            ret_val = process_folder(folder, args.save_loc, args.focus_graph_loc)
            if ret_val is None:
                continue
