...
- for each prediction/label, crop out the thumbnail and put it in its predicted/labelled class. Assign the `task.json` as an id (e.g. 0) if exporting multiple runs at once.
...
- for each cell in that map of corrections, find the the cell in its `tasks.json` file and change its class to the correct class
  - in the name `ring_2a4bee1ecf_0.png`, `ring` is the originally predicted class, `2a4bee1ecf` is the id of that thumbnail in its `task.json` file, and `0` is the `tasks.json` file that was used, as defined by `id_to_task_path.json`
