...
class FilePathObjectDetectionDataset(datasets.VisionDataset):
    def __init__(
        self,
        dataset_classes: List[str],
        image_path: Path,
...
def collate_batch(batch):
    inputs, labels, paths = zip(*batch)
    batched_inputs = torch.stack(inputs)
    batched_labels = torch.stack(labels)
    return batched_inputs, batched_labels, paths


def get_dataloader(
    image_path, label_path, normalize_images=False, batch_size=16, Sx=129, Sy=97
):
    dataset = FilePathObjectDetectionDataset(
        YOGO_CLASS_ORDERING,
        image_path,
        label_path,
        Sx,
