...
DATASET = input("String to filter for in experiment path (eg. 'disk6', 'disk7', 'disk8')\n")
...
mse_zoom = np.sqrt(np.mean(np.power(dff['mch_clinical'][filt]-dff['mch_estimate'][filt], 2)))
print(f"Mean squared err (zoomed in)= {mse_zoom}")

# Plot
plt.plot([MIN, MAX], [MIN, MAX], linestyle='--', c='orange', lw=1)
plt.scatter(dff['mch_clinical'], dff['mch_estimate'])
